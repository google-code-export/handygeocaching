...
public class Bluetooth implements DiscoveryListener
{
    ...
    public void searchDevices()
    {
        try
        {
            gui.get_frmConnecting().append("\nHledám dostupná Bluetooth zaøízení...");
            devices = new Vector();
    ...
    public void inquiryCompleted(int discType)
    {
        try
        {
            if (devices.size()==0)
            {
                gui.get_frmConnecting().append("\nŽádná Bluetooth zaøízení v dosahu nebo problém s Bluetooth");
            }
        }
        catch (Exception e)
        {
    ...
    public void serviceSearchCompleted(int transID, int responseCode)
    {
        ...
        {
            gui.showAlert("Váš telefon je pøipojen k jinému BT zaøízení nebo GPS modul již komunikuje s jiným mobilem.",AlertType.ERROR,gui.get_lstDevices());
        }
    }
}
    ...
    public void run()
    {
        ...
        while (searching)
        {
            try
            {
                ...
                if (timeout>20)
                {
                    main.get_frmConnecting().append("\nNepodaøilo se zahájit hledání Bluetooth služby");
                    searching = false;
                }
